...
interface FleurManagerLayoutContentProps {
  ...
  handleCreateInvoice: (customerName: string, invoiceCartItems: CartItem[], subtotalAfterItemDiscounts: number, paymentMethod: string, overallInvoiceDiscount: number, amountPaid: number, isGuestCustomer: boolean, employeeId: string, employeeName: string) => Promise<boolean>;
  handleAddProduct: (newProductData: Omit<Product, "id">) => Promise<void>;
  handleUpdateProduct: (productId: string, updatedProductData: Omit<Product, "id">) => Promise<void>;
...
export default function FleurManagerPage() {
  ...
  const handleDeleteCustomer = useCallback(async (customerId: string) => { try { await remove(ref(db, `customers/${customerId}`)); toast({ title: "Thành công", description: "Khách hàng đã được xóa.", variant: "default" }); } catch (error) { console.error("Error deleting customer:", error); toast({ title: "Lỗi", description: "Không thể xóa khách hàng. Vui lòng thử lại.", variant: "destructive" }); } }, [toast]);
  const onAddToCart = useCallback((item: Product) => { const existingItem = cart.find(cartItem => cartItem.id === item.id); const stockItem = inventory.find(i => i.id === item.id); if (!stockItem || stockItem.quantity <= 0) { toast({ title: "Hết hàng", description: `Sản phẩm "${item.name} ${item.color} ${item.quality} ${item.size} ${item.unit}" đã hết hàng!`, variant: "destructive" }); return; } if (existingItem) { if (existingItem.quantityInCart < stockItem.quantity) { setCart(prevCart => prevCart.map(cartItem => cartItem.id === item.id ? { ...cartItem, quantityInCart: cartItem.quantityInCart + 1 } : cartItem )); } else { toast({ title: "Số lượng tối đa", description: `Không đủ số lượng "${item.name} ${item.color} ${item.quality} ${item.size} ${item.unit}" trong kho (Còn: ${stockItem.quantity}).`, variant: "destructive" }); } } else { setCart(prevCart => [...prevCart, { ...item, quantityInCart: 1, itemDiscount: 0 }]); } }, [cart, inventory, toast, setCart]);
